...
contract WethDeposit {
  ...
  function deposit(uint64 user_id, uint32 exchange_id) public payable  {
    ...
    assembly {
      ...
      mstore(transfer_in_mem, /* fn_hash("user_deposit_to_session(uint64,uint32,uint32,uint64))") */ 0xf9ac1c2800000000000000000000000000000000000000000000000000000000)
      ...
      {
        let success := call(gas, 0x84f6451efe944ba67bedb8e0cf996fa1feb4031d, callvalue, transfer_in_mem, 164, transfer_out_mem, 0)
        if iszero(success) {
          mstore(32, 3)
          revert(63, 1)
        }
